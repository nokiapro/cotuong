...
function ensureClockUI(){
  try{ document.querySelectorAll('.river-side-clock').forEach(el=>{ try{ el.remove(); }catch(e){} }); }catch(e){}
  ...
  if(!document.getElementById('clockTopSlot')){
    const header = document.querySelector('.header');
    if(header){
      const slot = document.createElement('div');
      slot.className = 'board-clock-slot top';
      slot.id = 'clockTopSlot';
      slot.innerHTML = '<div class="wood-capsule board-clock" id="clockTopEl"><div class="footer-clock-side"><div class="footer-clock-icon black" id="clockTopIcon">將</div><span class="footer-clock-label" id="clockTopLabel">Bên Đen</span></div><span class="footer-clock-time" id="clockTopText">05:00</span></div>';
      header.appendChild(slot);
    }
  }
  if(!document.getElementById('clockBottomSlot')){
    const layout = document.querySelector('.layout') || document.getElementById('boardWrap');
    const slot = document.createElement('div');
    slot.className = 'board-clock-slot bottom';
    slot.id = 'clockBottomSlot';
    slot.innerHTML = '<div class="wood-capsule board-clock" id="clockBottomEl"><div class="footer-clock-side"><div class="footer-clock-icon red" id="clockBottomIcon">帥</div><span class="footer-clock-label" id="clockBottomLabel">Bên Đỏ</span></div><span class="footer-clock-time" id="clockBottomText">05:00</span></div>';
    if(layout && layout.parentNode) layout.parentNode.insertBefore(slot, layout.nextSibling);
    else document.body.appendChild(slot);
  }
  return document.getElementById('clockBottomEl');
}
...
function fillClockCapsule(prefix, side, sec){
  // prefix: Top | Bottom
  const icon = document.getElementById('clock'+prefix+'Icon');
  const label = document.getElementById('clock'+prefix+'Label');
  const time = document.getElementById('clock'+prefix+'Text');
  const el = document.getElementById('clock'+prefix+'El');
  if(!el) return;
  const isRed = side === 'red';
  if(icon){
    icon.textContent = isRed ? '帥' : '將';
    icon.className = 'footer-clock-icon ' + (isRed ? 'red' : 'black');
  }
  if(label) label.textContent = isRed ? 'Bên Đỏ' : 'Bên Đen';
  if(time) time.textContent = formatClockDisplay(sec);
  el.classList.toggle('red', isRed);
  el.classList.toggle('black', !isRed);
  el.classList.toggle('turn-active', state.turn === side && !_localClock.paused && !state.gameOver && !_localClock.infinite);
}
...
function renderClocks(){
  ...
  if(topSlot) topSlot.style.display = show ? '' : 'none';
  if(botSlot) botSlot.style.display = show ? '' : 'none';
  try{
  ...
  // Không lật: đỏ dưới, đen trên · Lật: đen dưới, đỏ trên
  ...
  fillClockCapsule('Bottom', bottomSide, bottomSide==='red' ? redSec : blackSec);
  fillClockCapsule('Top', topSide, topSide==='red' ? redSec : blackSec);
}
