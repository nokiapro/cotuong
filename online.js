...
async function fbCreateRoom(){
  ...
  if(!authed) return;
  fbSweepExpiredRooms(); try{ fbSweepExpiredReplays(); }catch(e){} try{ fbSweepExpiredGiftcodes(); }catch(e){}
...
async function fbSweepExpiredReplays(){
  try{
    ...
    if(!all) return;
    const now = Date.now();
    const ttl = (typeof REPLAY_TTL_MS !== 'undefined') ? REPLAY_TTL_MS : (12 * 60 * 60 * 1000);
    for(const id in all){
      const r = all[id];
      if(!r) continue;
      let exp = 0;
      if(typeof r.expiresAt === 'number' && r.expiresAt) exp = r.expiresAt;
      else if(typeof r.createdAt === 'number' && r.createdAt) exp = r.createdAt + ttl;
      else exp = 1;
      if(now > exp){
...
async function fbSweepExpiredGiftcodes(){
  try{
    if(!fb.db) return;
    const now = Date.now();
    const ttl = (typeof GIFTCODE_TTL_MS !== 'undefined') ? GIFTCODE_TTL_MS : (12 * 60 * 60 * 1000);
    const snap = await fb.db.ref('giftcodes').once('value');
    const all = snap.val() || {};
    for(const code of Object.keys(all)){
      const g = all[code] || {};
      let exp = 0;
      if(typeof g.expireTs === 'number' && g.expireTs) exp = g.expireTs;
      else if(typeof g.createdAt === 'number' && g.createdAt) exp = g.createdAt + ttl;
      else exp = 1;
      if(now > exp){
        try{ await fb.db.ref('giftcodes/'+code).remove(); }catch(e){}
        try{ await fb.db.ref('giftcodeClaims/'+code).remove(); }catch(e){}
      }
    }
    const cSnap = await fb.db.ref('giftcodeClaims').once('value');
    const claims = cSnap.val() || {};
    for(const code of Object.keys(claims)){
      const group = claims[code];
      if(!group || typeof group !== 'object'){
        try{ await fb.db.ref('giftcodeClaims/'+code).remove(); }catch(e){}
        continue;
      }
      try{
        const still = await fb.db.ref('giftcodes/'+code).once('value');
        if(!still.val()){
          await fb.db.ref('giftcodeClaims/'+code).remove();
          continue;
        }
      }catch(e){}
      for(const uid of Object.keys(group)){
        const row = group[uid];
        const ts = (row && typeof row === 'object') ? +(row.ts || 0) : (+row || 0);
        if(!ts || (now - ts) > ttl){
          try{ await fb.db.ref('giftcodeClaims/'+code+'/'+uid).remove(); }catch(e){}
        }
      }
    }
  }catch(err){
    console.warn('fbSweepExpiredGiftcodes', err);
  }
}
...
document.getElementById('cheatClaimUsesBtn')?.addEventListener('click', ()=>{ try{ closeDrawer(); }catch(e){} claimCheatUsesDaily(); });
...
/* ========== Thông báo toàn website (admin) ========== */
let _siteAnnounceLastId = '';
let _siteAnnounceBusy = false;
let _siteAnnounceTimers = [];

function clearSiteAnnounceTimers(){
  _siteAnnounceTimers.forEach(id=>{ try{ clearTimeout(id); }catch(e){} });
  _siteAnnounceTimers = [];
}

function hideSiteAnnounceBar(){
  const bar = document.getElementById('siteAnnounceBar');
  if(!bar) return;
  bar.classList.remove('is-show');
  bar.hidden = true;
  const txt = document.getElementById('siteAnnounceText');
  if(txt){
    txt.classList.remove('is-run');
    txt.style.animationDuration = '';
    txt.textContent = '';
  }
}

function playSiteAnnounceOnce(text){
  return new Promise(resolve=>{
    const bar = document.getElementById('siteAnnounceBar');
    const txt = document.getElementById('siteAnnounceText');
    if(!bar || !txt){ resolve(); return; }
    bar.hidden = false;
    bar.classList.add('is-show');
    txt.classList.remove('is-run');
    txt.textContent = text;
    const dur = Math.min(28, Math.max(8, 6 + String(text).length * 0.12));
    txt.style.animationDuration = dur + 's';
    void txt.offsetWidth;
    txt.classList.add('is-run');
    const tid = setTimeout(()=>{ hideSiteAnnounceBar(); resolve(); }, dur * 1000 + 80);
    _siteAnnounceTimers.push(tid);
  });
}

async function runSiteAnnounceSequence(payload){
  if(_siteAnnounceBusy) return;
  const text = (payload && payload.text || '').trim();
  if(!text) return;
  const runs = Math.max(1, Math.min(5, +(payload.runs || 3)));
  const gapMs = Math.max(0, +(payload.gapMs != null ? payload.gapMs : 20000));
  _siteAnnounceBusy = true;
  clearSiteAnnounceTimers();
  try{
    for(let i = 0; i < runs; i++){
      await playSiteAnnounceOnce(text);
      if(i < runs - 1 && gapMs > 0){
        await new Promise(r=>{
          const tid = setTimeout(r, gapMs);
          _siteAnnounceTimers.push(tid);
        });
      }
    }
  }catch(e){ console.warn('siteAnnounce', e); }
  finally{
    hideSiteAnnounceBar();
    _siteAnnounceBusy = false;
  }
}

function handleSiteAnnouncePayload(val){
  if(!val || !val.text) return;
  const id = val.id || String(val.ts || '');
  if(!id || id === _siteAnnounceLastId) return;
  const ts = +(val.ts || 0);
  if(ts && (Date.now() - ts) > 3 * 60 * 1000){
    _siteAnnounceLastId = id;
    return;
  }
  _siteAnnounceLastId = id;
  runSiteAnnounceSequence(val);
}

async function initSiteAnnounceListener(){
  try{
    if(typeof fbConfigured === 'function' && !fbConfigured()) return;
    if(typeof fbAvailable === 'function' && !fbAvailable()){
      try{ await fbInit(); }catch(e){}
    }
    try{ if(typeof fbEnsureAuthOptional === 'function') await fbEnsureAuthOptional(); }catch(e){}
    try{ if(typeof fbInit === 'function') await fbInit(); }catch(e){}
    if(!fb.db) return;
    // Mọi client (khách / kỳ thủ / đang chơi hay không) đều nhận thông báo toàn site
    fb.db.ref('admin/siteAnnounce').off('value');
    fb.db.ref('admin/siteAnnounce').on('value', snap=>{
      try{ handleSiteAnnouncePayload(snap.val()); }catch(e){ console.warn(e); }
    });
  }catch(e){ console.warn('initSiteAnnounceListener', e); }
}

try{
  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', ()=>{ setTimeout(initSiteAnnounceListener, 600); });
  } else {
    setTimeout(initSiteAnnounceListener, 600);
  }
}catch(e){}
