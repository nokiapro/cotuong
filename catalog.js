...
// Logo bàn không phải CLB bóng đá (vào tab Giao diện)
const THEME_LOGOS = {
  'hoicotuong': {
...
// Seed THEMES + THEME_META cho logo bàn
(function seedLogoThemes(){
  if(typeof THEMES === 'undefined' || typeof THEME_META === 'undefined') return;
  if(typeof THEME_LOGOS !== 'undefined'){
    Object.keys(THEME_LOGOS).forEach(id=>{
      const L = THEME_LOGOS[id];
      if(!THEMES[id]){
        THEMES[id] = {
          wood1:L.wood1, wood2:L.wood2, wood3:L.wood3,
          grain1:L.grain1, grain2:L.grain2, grain3:L.grain3,
          accent:L.accent, accentGlow:L.accentGlow
        };
      }
      if(!THEME_META[id]){
        THEME_META[id] = { name: L.name, price: L.price||80, logo: true, slug: L.slug };
      } else {
        THEME_META[id].logo = true;
        THEME_META[id].slug = L.slug || THEME_META[id].slug;
      }
    });
  }
})();
