...
async function generateBracket(){
  ...
  try{
    ...
    // Tranh hạng 3 khi ≥ 4 người (2 trận bán kết)
    let third = null;
    if(rounds >= 2){
      third = {
        id: 'se_'+tnId+'_3rd', tournamentId: tnId, kind: 'se_3rd', bracket: 'third',
        round: rounds, index: 0,
        red: null, black: null, redName: 'TBD', blackName: 'TBD',
        redScore: null, blackScore: null,
        roomCode: '', status: 'pending', winner: null, table: 1,
        nextMatchId: null, nextSlot: null
      };
      tcData.matches[third.id] = third;
      // Losers of semi (round rounds-1) → third place
      const semis = byRound[rounds - 1] || [];
      semis.forEach((sm, i)=>{
        sm.loserNextMatchId = third.id;
        sm.loserNextSlot = (i % 2 === 0) ? 'red' : 'black';
      });
    }
    ...
    if(third) writes.push(tcSet('matches/'+third.id, third));
    await Promise.all(writes);
    ...
    setAdminStatus('Sơ đồ SE: '+n+' kỳ thủ · bracket '+size+' · '+rounds+' vòng'+(third?' · tranh H3':'')+(advanced?' · auto-BYE '+advanced:''), 'ok');
...
async function claimGuestPresence(){
  ...
  try{
    ...
    let user = null;
    try{ user = await fbEnsureAuthOptional(); }catch(e){}
    if(!user){
      try{
        if(typeof fbEnsureAuth === 'function') await fbEnsureAuth();
    ...
    if(!user || !user.uid){
      ...
      try{
        await showAccessBlockedPopup(0, 0, 'Không xác thực được phiên khách. Thử tải lại trang hoặc đăng nhập kỳ thủ.');
      }catch(e){}
      return false;
    }
    ...
    const now = Date.now();
    // Chỉ đếm guest còn "sống" trong 3 phút
    let liveCount = 0;
    Object.keys(guests).forEach(k=>{
      if(k === user.uid) return;
      const g = guests[k];
      const ts = (g && g.ts) ? +g.ts : 0;
      if(!ts || (now - ts) < 180000) liveCount++;
    });
    if(guests[user.uid]){
      accessGatePassed = true;
    ...
    accessGatePassed = false;
    try{
      await showAccessBlockedPopup(0, 0, 'Không kiểm tra được số khách online. Vui lòng quay lại sau.');
    }catch(e){}
    return false;
  }
}
...
function closeDiagramPick(){
  document.getElementById('diagramPickOverlay')?.classList.remove('show');
  diagramPickIndex = -1;
}
...
async function buildBracketFromSlots(){
  ...
  try{
    ...
    if(rounds >= 2){
      ...
      tcData.matches[third.id] = third;
      (byRound[rounds-1]||[]).forEach((sm,i)=>{
        sm.loserNextMatchId = third.id;
        sm.loserNextSlot = (i%2===0)?'red':'black';
      });
    }

    // Pair seed high vs low within bracket positions
    for(let i=0;i<size/2;i++){
      const a = slots[i];
      ...
      row.red = a ? a.id : null;
      row.black = b ? b.id : null;
      row.redName = a ? (a.name||a.code) : 'BYE';
      row.blackName = b ? (b.name||b.code) : 'BYE';
      if(!a || !b){
        row.status = 'finished';
        row.winner = !a ? (b?b.id:null) : (a?a.id:null);
        if(row.winner===row.red){ row.redScore=1; row.blackScore=0; }
    ...
    if(third) writes.push(tcSet('matches/'+third.id, third));
    await Promise.all(writes);
    await tcUpdate('tournaments/'+tnId, { format:'single_elim', bracketSize:size, slotSeed: diagramSlots.map(p=>p&&p.id) });
    ...
    diagramSetStatus('Đã tạo bracket '+size+' · '+filled.length+' kỳ thủ · '+rounds+' vòng'+(third?' · H3':'')+'.');
...
function buildBracketTreeHtml(tnId, matchesObj){
  ...
  if(!se.length && !third) return '<div class="admin-empty">Chưa có nhánh — hãy sinh nhánh ở tab Nhánh & Bàn.</div>';
  ...
  function lineHtml(m, side){
    ...
    const name = side === 'red' ? (m.redName || 'TBD') : (m.blackName || 'TBD');
  ...
  if(third){
    html += '<div class="bv-third-wrap"><div class="bv-round-title">Tranh hạng 3</div>'+matchCard(third, 'bv-third')+'</div>';
  }
  html += '</div>';
  return html;
}
...
async function openPublicBracketModal(){
  ...
  try{
    if(typeof fbAvailable === 'function' && fbAvailable()){
      try{ await fbEnsureAuth(); }catch(e){}
      const [tnSnap, mSnap] = await Promise.all([
        fb.db.ref('tournaments').once('value'),
        fb.db.ref('matches').once('value')
      ]);
      ...
      if(sel){
        const cur = sel.value;
        sel.innerHTML = '<option value="">— Chọn giải —</option>' + Object.values(tns).map(t=>
          `<option value="${t.id}">${t.name||t.code} (${t.code||t.id})</option>`
        ).join('');
        if(cur) sel.value = cur;
        else if(Object.keys(tns).length) sel.selectedIndex = 1;
      }
      if(box) box.innerHTML = buildBracketTreeHtml(sel && sel.value, matches);
...
function renderQuickMatchSlots(){
  ...
  box.innerHTML = qmSlots.map((p,i)=>{
    if(p){
      ...
      return '<div class="dslot filled qm-dslot" data-qm-slot="'+i+'">'
        ...
        + (av ? '<img class="dslot-av" src="'+av+'" alt="">' : '<span class="dslot-av ph">👤</span>')
        + '<span class="dslot-name">'+(p.name||p.code||p.id)+'</span>'
        + '<button type="button" class="dslot-x" data-qm-clear="'+i+'" title="Xóa">×</button>'
        + '</div>';
    }
  ...
  box.querySelectorAll('[data-qm-slot]').forEach(el=>{
    el.addEventListener('click', (e)=>{
      if(e.target.closest('[data-qm-clear]')) return;
      openQmPick(+el.getAttribute('data-qm-slot'));
    });
  });
  box.querySelectorAll('[data-qm-clear]').forEach(btn=>{
    btn.addEventListener('click', (e)=>{
      e.stopPropagation();
      qmSlots[+btn.getAttribute('data-qm-clear')] = null;
      renderQuickMatchSlots();
    });
  });
}
let qmPickIndex = -1;
function openQmPick(index){
  qmPickIndex = index;
  diagramPickIndex = -1;
  const ov = document.getElementById('diagramPickOverlay');
  const meta = document.getElementById('diagramPickMeta');
  ...
  if(meta) meta.textContent = 'Ô seed #'+(index+1);
  if(search) search.value = '';
  if(ov){ ov.dataset.mode = 'qm'; ov.classList.add('show'); }
  renderQmPickList();
}
    ...
    for(let i=0;i<br/2;i++){
      ...
      if(a && b){
        writes.push(fb.db.ref('rooms/'+code).set({
          ...
          gameOver:false, createdAt:Date.now(), quickMatch:true,
          seats:{
...
async function renderPollResults(){
  ...
  try{
    await adminEnsureFb();
    const snap = await fb.db.ref('admin/polls').limitToLast(10).once('value');
    const val = snap.val()||{};
    const list = Object.keys(val).map(k=>({id:k,...val[k]})).sort((a,b)=>(b.ts||0)-(a.ts||0));
    if(!list.length){ box.innerHTML = '<div class="admin-empty">Chưa có poll.</div>'; return; }
    box.innerHTML = list.map(p=>{
