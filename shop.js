...
const REPLAY_TTL_MS = 12 * 60 * 60 * 1000; // 12 giờ
const GIFTCODE_TTL_MS = 12 * 60 * 60 * 1000; // giftcodes + claims 12 giờ

const CHEAT_DAILY_LIMIT = 20;
const CHEAT_BUY_USES = 10;
const CHEAT_BUY_COST = 30;
const CHEAT_FREE_CLAIM = 5; // nhận miễn phí mỗi ngày
// Thưởng cố định thắng máy (cấp 1..10)
const AI_WIN_REWARD = [0, 8, 12, 18, 28, 42, 60, 85, 120, 170, 240];
// % lãi trên tiền cược khi thắng
const AI_PROFIT_PCT = [0, 0.12, 0.18, 0.24, 0.32, 0.42, 0.55, 0.70, 0.85, 1.00, 1.20];
const AI_STAKE_BANKROLL_FRAC = 0.40;
...
let coinState = { coins: 0, unlocked: ['wood','jade','rosewood','marble'], lastCheckIn: '', inventory: {}, active: {}, cheatDate: '', cheatUsed: 0, cheatBonus: 0, lastCheatClaim: '' };
...
async function loadCoinStateFromPlayer(){
  ...
  try{
    ...
    if(ident.kind === 'player'){
      ...
      if(+p.cheatUses > 0) coinState.cheatBonus += Math.max(0, +p.cheatUses);
      coinState.lastCheatClaim = p.lastCheatClaim || '';
      ...
      if(+w.cheatUses > 0) coinState.cheatBonus += Math.max(0, +w.cheatUses);
      coinState.lastCheatClaim = w.lastCheatClaim || '';
  ...
  try{
    ...
    if(ident.kind === 'player'){
      await fb.db.ref('players/'+ident.id).update({
        ...
        lastCheatClaim: coinState.lastCheatClaim || null,
