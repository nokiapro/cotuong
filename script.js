...
function canUseAiAnalysis(){
  return state.mode === 'pve' && !state.online.active && !(state.replay && state.replay.active);
}

function scoreMoveOnBoard(board, move, color, depth){
  const nb = cloneBoard(board);
  nb[move.to.r][move.to.c] = nb[move.from.r][move.from.c];
  nb[move.from.r][move.from.c] = null;
  const opp = color === 'red' ? 'black' : 'red';
  return minimax(nb, depth, -Infinity, Infinity, opp);
}

function findBestMoveAnalysis(board, color, depth){
  const moves = allLegalMoves(board, color);
  if(!moves.length) return null;
  let best = null;
  let bestScore = color === 'red' ? -Infinity : Infinity;
  for(const m of moves){
    const sc = scoreMoveOnBoard(board, m, color, depth);
    if(color === 'red'){
...
function analyzeLastHumanMove(){
  if(!canUseAiAnalysis()) {
    setAiAnalysisUI(null, true);
    return;
  }
  if(!state.history.length) return;
  const last = state.history[state.history.length - 1];
  if(!last || !last.piece) return;
  // Only analyze human's moves
  if(last.piece.color !== state.humanColor) {
    return;
  }
  // Reconstruct board before last move
  const boardBefore = cloneBoard(state.board);
  // undo last on clone: put piece back, restore capture
  boardBefore[last.from.r][last.from.c] = last.piece;
  boardBefore[last.to.r][last.to.c] = last.captured || null;

  const color = last.piece.color;
  const depth = 2; // light analysis
  let playedScore, best;
  try{
    playedScore = scoreMoveOnBoard(boardBefore, { from: last.from, to: last.to }, color, depth);
    best = findBestMoveAnalysis(boardBefore, color, depth);
  }catch(e){
    setAiAnalysisUI({ text: 'Không phân tích được nước này.', tier: 'neutral' });
    return;
  }
  if(!best) return;

  // Score from red's perspective in evaluate; for human black, invert comparison
  const humanIsRed = color === 'red';
  let delta = humanIsRed ? (playedScore - best.score) : (best.score - playedScore);
  // delta <= 0 means played is as good or worse than best for the side

  const notation = typeof moveNotation === 'function'
    ? moveNotation(last.piece, last.from, last.to)
    : (last.piece.type + ' ' + last.from.r + ',' + last.from.c + '→' + last.to.r + ',' + last.to.c);

  let tier = 'ok', label = 'Nước ổn', tip = 'Không sai lớn so với gợi ý máy.';
  if(delta >= -30){
    tier = 'best'; label = 'Nước tốt'; tip = 'Gần với nước máy chọn.';
  } else if(delta >= -120){
    tier = 'ok'; label = 'Nước ổn'; tip = 'Chấp nhận được, còn nước mạnh hơn.';
  } else if(delta >= -280){
    tier = 'inaccuracy'; label = 'Nước yếu'; tip = 'Có nước tốt hơn rõ rệt.';
  } else {
    tier = 'blunder'; label = 'Sai lầm'; tip = 'Nước này mất nhiều điểm thế cờ.';
  }

  let bestNote = '';
  if(best.move){
    const bp = boardBefore[best.move.from.r][best.move.from.c];
    if(bp && typeof moveNotation === 'function'){
      bestNote = moveNotation(bp, best.move.from, best.move.to);
    }
  }

  setAiAnalysisUI({
    tier, label, tip, notation, bestNote, delta: Math.round(delta)
  });
}

function setAiAnalysisUI(data, hidden){
  const box = document.getElementById('aiAnalysisBox');
  if(!box) return;
  if(hidden || !canUseAiAnalysis()){
    box.style.display = 'none';
    return;
  }
  box.style.display = '';
  if(!data){
    box.querySelector('.ai-analysis-body').innerHTML = '<div class="admin-empty">Đánh một nước để xem phân tích.</div>';
    return;
  }
  const tierClass = 'tier-' + (data.tier || 'ok');
  box.querySelector('.ai-analysis-body').innerHTML =
    '<div class="ai-tier '+tierClass+'">'+data.label+'</div>'+
    '<div class="ai-move">Bạn đi: <b>'+(data.notation||'')+'</b></div>'+
    (data.bestNote && data.tier !== 'best' ? '<div class="ai-best">Gợi ý: <b>'+data.bestNote+'</b></div>' : '')+
    '<div class="ai-tip">'+(data.tip||'')+'</div>';
}
...
const ACHIEVEMENTS = {
  win10: { id:'win10', name:'Thắng 10 ván', icon:'🏆', desc:'Thắng 10 ván cờ', check: s => (s.wins||0) >= 10 },
  checkin7: { id:'checkin7', name:'Điểm danh 7 ngày', icon:'📅', desc:'Chuỗi điểm danh 7 ngày', check: s => (s.checkInStreak||0) >= 7 },
  theme5: { id:'theme5', name:'Sưu tầm 5 theme', icon:'🎨', desc:'Mở khóa 5 giao diện', check: s => (s.unlockedThemes||0) >= 5 },
  win50: { id:'win50', name:'Kỳ thủ cứng', icon:'♟️', desc:'Thắng 50 ván', check: s => (s.wins||0) >= 50 },
  checkin30: { id:'checkin30', name:'Gắn bó 30 ngày', icon:'🔥', desc:'Chuỗi điểm danh 30 ngày', check: s => (s.checkInStreak||0) >= 30 },
  firstWin: { id:'firstWin', name:'Chiến thắng đầu', icon:'⭐', desc:'Thắng ván đầu tiên', check: s => (s.wins||0) >= 1 },
  shopper: { id:'shopper', name:'Mạnh tay', icon:'🛒', desc:'Mua 5 vật phẩm', check: s => (s.purchases||0) >= 5 },
  social: { id:'social', name:'Giao hữu', icon:'🤝', desc:'Có ít nhất 3 bạn', check: s => (s.friendCount||0) >= 3 }
};

function getAchievementStats(){
  const unlocked = (coinState && coinState.unlocked) ? coinState.unlocked.length : 0;
  const freeThemes = ['wood','jade','rosewood','marble'];
  const paidUnlocked = (coinState && coinState.unlocked) ? coinState.unlocked.filter(t => !freeThemes.includes(t)).length : 0;
  return {
    wins: (coinState && coinState.wins) || 0,
    checkInStreak: (coinState && coinState.checkInStreak) || 0,
    unlockedThemes: paidUnlocked + (coinState && coinState.unlocked ? coinState.unlocked.filter(t=>freeThemes.includes(t)).length : 0),
...
function renderAchievementsUI(){
  ...
  Object.values(ACHIEVEMENTS).forEach(a=>{
    ...
    div.className = 'achievement-item' + (have.has(a.id) ? ' unlocked' : ' locked');
    div.innerHTML = '<span class="ach-icon">'+a.icon+'</span><div class="ach-meta"><b>'+a.name+'</b><span>'+a.desc+'</span></div>'+
      (have.has(a.id) ? '<span class="ach-done">✓</span>' : '<span class="ach-lock">🔒</span>');
    box.appendChild(div);
  });
}
...
async function loadReplayFromId(id){
  ...
  try{
    ...
    if(!data || !data.moves || !data.moves.length){
      alert('Không tìm thấy replay «'+id+'».');
      return;
    }
    ...
    state.history = [];
    state.lastMove = null;
    state.replay = { active:true, moves: data.moves, index:0, id };
    ...
    if(bar){
      ...
      if(info) info.textContent = 'Replay '+id+' · 0/'+data.moves.length+(data.winner? ' · '+(data.winner==='red'?'Đỏ':'Đen')+' thắng':'');
    }
    try{ showCoinPopup({ icon:'🎬', title:'Chế độ xem lại', html:'<div class="coin-popup-hint">Dùng nút ▶ / ◀ để xem từng nước. Link: ?replay='+id+'</div>' }); }catch(e){}
  }catch(e){
...
function replayStep(dir){
  ...
  if(dir > 0){
    if(state.replay.index >= moves.length) return;
    const m = moves[state.replay.index];
    doMove(m.from, m.to, { silent:true, noOnline:true, fromReplay:true });
    state.replay.index++;
  } else {
    if(state.replay.index <= 0) return;
    state.replay.index--;
    // rebuild from start
    state.board = initialBoard();
    state.turn = 'red';
    ...
    state.lastMove = null;
    state.gameOver = false;
    try{ clearCaptured && clearCaptured(); }catch(e){}
    const hist = document.getElementById('historyList');
    if(hist) hist.innerHTML = '';
    for(let i=0;i<state.replay.index;i++){
      const m = moves[i];
      doMove(m.from, m.to, { silent:true, noOnline:true, fromReplay:true });
    }
    renderPieces();
  ...
  const info = document.getElementById('replayInfo');
  if(info) info.textContent = 'Replay · '+state.replay.index+'/'+moves.length;
  updateStatus();
}
...
const HOLIDAY_THEMES = {
  ...
  tet: {
    ...
    // approximate lunar - unlock Jan 20 - Feb 20
    range:[[1,20],[2,20]],
  ...
  halloween: {
    id:'event_halloween', name:'Halloween', price:0, freeEvent:true,
    range:[[10,20],[10,31]],
    wood1:'#1a0a2e', wood2:'#0d0618', wood3:'#050208',
    grain1:'#ff6d00', grain2:'#9c27b0', grain3:'#ffab40',
    accent:'#ff6d00', accentGlow:'#ffab40',
    badge:'🎃'
  }
};
...
function isThemeUnlocked(id){
  ...
  if(meta && meta.freeEvent && isHolidayActive(HOLIDAY_THEMES[meta.eventKey])) return true;
...
function rebuildShopItems(){ try{ SHOP_ITEMS = buildShopCatalog(); }catch(e){} }
let shopTab = 'all';
let shopPage = 0;
...
function doMove(from, to, opts={}){
  ...
  try{
    if(canUseAiAnalysis() && !(opts && (opts.fromReplay || opts.silent))){
      const last = state.history[state.history.length-1];
      if(last && last.piece && last.piece.color === state.humanColor){
        setTimeout(()=>{ try{ analyzeLastHumanMove(); }catch(e){} }, 30);
      }
    } else {
      setAiAnalysisUI(null, true);
    }
    ...
    if(list){
      ...
      if(!ids.length) list.innerHTML = '<div class="admin-empty">Chưa có bạn.</div>';
        ...
        ids.forEach(fid=>{
          ...
          challenge.className = 'action-btn';
          challenge.textContent = 'Thách';
          challenge.title = 'Thách đấu nhanh + cược';
          challenge.addEventListener('click', ()=> quickChallengeFriend(f.code||'', fid));
...
function updateAiLevelBoxVisibility(){
  ...
  const an = document.getElementById('aiAnalysisBox');
  if(an) an.style.display = (state.mode==='pve' && !state.online.active) ? '' : 'none';
  if(!(state.mode==='pve' && !state.online.active)) setAiAnalysisUI(null, true);
}
