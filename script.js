...
/* ---------------- Board themes ---------------- */
/* THEMES/META/FLAGS/CLUBS → catalog.js */
...
async function evaluateAchievements(silent){
  ...
  if(newly.length){
    ...
    if(!silent){
      // Chỉ hiện popup 1 lần / thành tựu trong phiên (tránh spam khi mua liên tục)
...
async function loadReplayFromId(id){
  ...
  try{
    ...
    if(exp && Date.now() > exp){
      ...
      alert('Replay «'+id+'» đã hết hạn (24 giờ) và đã bị xóa.');
      return;
    }
    ...
    updateReplayUI();
    updateStatus();
    // Không bật popup che nút X — status đủ
...
/** Cộng dồn thống kê coin theo ngày: admin/coinDaily/{YYYY-MM-DD}/{bucket} */
...
const REPLAY_TTL_MS = 24 * 60 * 60 * 1000; // replay Firebase hết hạn sau 24h

const CHEAT_DAILY_LIMIT = 20;
const CHEAT_BUY_USES = 10;
const CHEAT_BUY_COST = 30;
/** Thưởng thắng máy theo cấp 1..10 (không cần cược) */
const AI_WIN_REWARD = [0, 2, 3, 4, 5, 7, 9, 12, 15, 20, 26];
/** % lãi trên số cược khi thắng (cấp khó lãi cao hơn — không farm cấp dễ) */
const AI_PROFIT_PCT = [0, 0.05, 0.08, 0.10, 0.14, 0.20, 0.28, 0.38, 0.50, 0.65, 0.85];
/** Tối đa cược = 25% số dư (tối thiểu 1 nếu còn coin) */
const AI_STAKE_BANKROLL_FRAC = 0.25;

/** Cửa hàng — SHOP_GIFTS + theme từ catalog.js */
function buildShopCatalog(){
  ...
  const themeEmoji = {
    ...
    ember:'🌋', ocean:'🛳️', bamboo:'🎋', golden:'🪙', frost:'☃️', dragon:'🐲', halloween:'🎃'
  };
...
/** Gộp THEMES + THEME_CLUBS từ catalog */
...
/** Chỉ dùng cho logo CLB: line/glyph = 'white'|'black' (catalog) */
...
/** Theme thường: trả kẻ bàn + chữ quân về mặc định gốc */
...
function applyTheme(themeId, opts={}){
  ...
  // line/glyph chỉ áp dụng theme logo CLB
  try{
...
function updateBoardFlag(themeId){
  ...
  if(club){
    ...
    // Filter: viền trắng mỏng theo contour logo + đổ bóng nhẹ
    if(defs){
      const oldF = document.getElementById('clubLogoOutlineFilter');
      if(oldF) oldF.remove();
      const outlineOn = club.outline !== false; // mặc định bật
...
function refreshThemeLocks(){
  document.querySelectorAll('.theme-swatch').forEach(btn=>{
    ...
    btn.title = (THEME_META[id]?.name || id) + (price ? ' (Đã mở)' : ' (Miễn phí)');
...
async function loadCoinStateFromPlayer(){
  ...
  // Khôi phục theme đã chọn (sau khi biết unlocked)
  try{
...
async function createGiftCode(itemId){
  ...
  try{
    await tcEnsureFb();
    // Mã dạng XK-XXXXXXXX — KHÔNG trừ đồ trong kho (chỉ sinh mã chia sẻ)
...
async function listItemForAuction(itemId){
  ...
  // Popup form đấu giá (không dùng prompt trình duyệt)
  const form = await new Promise(resolve=>{
    ...
    // capture phase để chạy trước listener mặc định
...
async function placeBid(auctionId){
  ...
  try{
    ...
    // refresh auction once more before commit
...
function loadSavedTheme(){
  ...
  // Chưa load Firebase: vẫn apply tạm (force); sau loadCoinState sẽ apply lại đúng unlocked
...
function getAiBetStakeFromUi(){
  const el = document.getElementById('aiBetStake');
  if(!el) return 0;
  return Math.max(0, Math.min(9999, Math.floor(+el.value || 0)));
}
...
/** Gợi ý mức cược theo số dư + cấp (chiến thuật: ~8–15% bankroll, không quá 25%) */
function suggestedAiStake(level, coins){
  const c = Math.max(0, Math.floor(+coins || 0));
  if(c < 5) return 0;
  const lv = Math.min(10, Math.max(1, +(level||5)));
  // Cấp dễ: cược nhỏ; cấp khó: có thể cược % cao hơn một chút nếu chắc tay
...
/** Khóa coin cược vs máy khi bắt đầu ván PVE */
async function lockAiStakeIfNeeded(){
  ...
  try{
    await loadCoinStateFromPlayer();
    let useStake = stake;
    const cap = Math.max(0, Math.floor(+(coinState.coins||0) * AI_STAKE_BANKROLL_FRAC));
    if(useStake > +(coinState.coins||0)){
      setAiBetStatus('Không đủ coin (có '+coinState.coins+', cần '+useStake+'). Đặt 0 hoặc giảm cược.', true);
      state.aiStake = 0;
      return true;
    }
    if(useStake > cap && cap > 0){
      // không chặn cứng — cảnh báo nhưng vẫn cho cược (người chơi chủ động)
      setAiBetStatus('Cược '+useStake+' > gợi ý an toàn '+cap+' (25% số dư).', true);
    }
    coinState.coins -= useStake;
    state.aiStake = useStake;
    state.aiStakeLocked = useStake;
    await saveCoinStateToPlayer();
    refreshThemeLocks();
    setAiBetStatus('Đã khóa '+useStake+' coin · lãi thắng ~'+aiProfitForStake(useStake, state.aiLevel)+' + thưởng '+aiWinRewardForLevel(state.aiLevel)+'.', false);
    showCoinPopup({
      icon:'🤖',
      title:'Thách máy',
      html:'<ul class="coin-popup-list"><li>Cược: <b>'+useStake+'</b> coin (đã khóa)</li><li>Cấp máy: <b>'+state.aiLevel+'</b></li><li>Lãi nếu thắng: <b>+'+aiProfitForStake(useStake, state.aiLevel)+'</b> + thưởng <b>+'+aiWinRewardForLevel(state.aiLevel)+'</b></li><li>Thua: mất '+useStake+' coin</li></ul>'
    });
  }catch(e){
    state.aiStakeLocked = 0;
    state.aiStake = 0;
...
async function settlePveCoins(resultWinner){
  ...
  try{
    ...
    if(resultWinner === state.humanColor){
      ...
      // thua: mất locked (đã trừ), không cộng
      title = 'Thua máy';
      html = locked
    ...
    try{
      if(resultWinner === state.humanColor){
        ...
        await logCoinDaily('pveOut', paid); // thưởng hệ thống chi
        if(locked > 0) await logCoinDaily('pveStake', locked); // hoàn cược (không tính là chi)
...
function ensureCheatDay(){
  ...
  if(coinState.cheatDate !== d){
    ...
    // cheatBonus giữ lại (đã mua) — hoặc reset: user asked daily reset of uses, bonus could stay
  }
  return d;
}
...
function refreshCheatUsesUI(){
  ...
  if(isCheatUnlimited()){
    ...
    if(st){ st.textContent = 'Superadmin — gian lận không giới hạn.'; st.className = 'online-status live'; }
...
/** Trừ 1 lượt gian lận. Trả false nếu hết lượt. */
...
async function resetGame(opts={}){
  stopReplayIfActive();
  // Nếu đang có cược máy khóa dở (thoát giữa chừng) — hoàn lại
  ...
  try{ updateStatus(); }catch(e){}
  try{ updateUndoBtn(); }catch(e){}
  // skipAiStake: không tự cược (thoát xem lại / về đầu bàn sạch)
  if(!opts.skipAiStake){
    try{ await lockAiStakeIfNeeded(); }catch(e){}
  } else {
    state.aiStake = 0;
    // không hoàn locked ở đây nếu đã hoàn phía trên; chỉ tránh lock mới
...
document.getElementById('modalBtn').addEventListener('click', ()=> resetGame({ skipAiStake: true }));
...
async function renderAdminCoins(){
  ...
  try{
    ...
    // thống kê ngày từ ledger + checkIns
    let day = {};
    try{
    ...
    const pveNet = pveOut; // hiển thị thưởng chi ra; có thể hiện pveIn riêng
...
async function loadConfigAndInit(){
  ...
  if(fbConfigured()){
    setInterval(()=>{
      (async ()=>{
        try{ await fbEnsureAuth(); fbSweepExpiredRooms(); fbSweepExpiredReplays(); }catch(err){}
      })();
    }, 60000);
    fbEnsureAuth().then(u=>{
