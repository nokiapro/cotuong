...
function isBoardFlipped(){ return !!state.boardFlipped; }
function boardX(c){
  const cols = (typeof COLS !== 'undefined' ? COLS : 9);
  return MARGIN + (isBoardFlipped() ? (cols - 1 - c) : c) * CELL;
}
function boardY(r){
  const rows = (typeof ROWS !== 'undefined' ? ROWS : 10);
  return MARGIN + (isBoardFlipped() ? (rows - 1 - r) : r) * CELL;
}
function setBoardOrientation(flipped){
  const next = !!flipped;
  if(!!state.boardFlipped === next){
    // vẫn rebuild nếu class lệch
    try{
      const wrap = document.getElementById('boardWrap') || document.querySelector('.board-wrap');
      if(wrap) wrap.classList.toggle('board-flipped', next);
    }catch(e){}
    return;
  }
  state.boardFlipped = next;
  try{
    const wrap = document.getElementById('boardWrap') || document.querySelector('.board-wrap');
    if(wrap) wrap.classList.toggle('board-flipped', next);
  }catch(e){}
  ...
  try{
    if(typeof applyTheme === 'function' && state.themeId) applyTheme(state.themeId, { force:true });
    else if(typeof updateBoardFlag === 'function') updateBoardFlag(state.themeId || 'wood');
  }catch(e){}
  try{ if(typeof renderClocks === 'function') renderClocks(); }catch(e){}
}
function syncBoardOrientationFromRole(){
  if(state.online && state.online.active){
    if(state.online.spectator){
      const mode = (typeof spectatorViewMode !== 'undefined') ? spectatorViewMode : 'red';
      setBoardOrientation(mode === 'black');
      return;
    }
    // Người chơi: đỏ dưới / đen dưới theo màu cầm
    setBoardOrientation(state.online.color === 'black');
    return;
  }
  if(state.mode === 'pve' && state.humanColor === 'black'){ setBoardOrientation(true); return; }
...
function createPieceElement(p){
  ...
  try{ group.dataset.color = p.color; group.dataset.type = p.type; }catch(e){}
  ...
  let svgUrl = null;
  try{
    const setId = (typeof getActivePieceSetId === 'function') ? getActivePieceSetId() : (function(){ try{ return localStorage.getItem('cotuong_piece_set'); }catch(e){ return null; } })();
    const unlocked = !setId ? false : (typeof isPieceSetUnlocked === 'function' ? isPieceSetUnlocked(setId) : true);
    if(setId && unlocked && setId !== 'default'){
      if(typeof pieceSvgUrl === 'function') svgUrl = pieceSvgUrl(setId, p.color, p.type);
      else {
        const map = { general:'tuong', advisor:'si', elephant:'tinh', horse:'ma', chariot:'xe', cannon:'phao', soldier:'tot' };
        const key = map[p.type];
        if(key) svgUrl = 'https://raw.githubusercontent.com/nokiapro/cotuong/refs/heads/main/quanco/'+setId+'/'+(p.color==='red'?'do':'den')+'_'+key+'.svg';
      }
    }
  }catch(e){}
  ...
  base.setAttribute('class','piece-base-classic');
  ...
  const glyphsMap = (typeof GLYPHS !== 'undefined' && GLYPHS) ? GLYPHS : {};
  ...
  const gText = (glyphsMap[p.color] && glyphsMap[p.color][p.type]) || (fallbackGlyphs[p.color]||{})[p.type] || '';
  ...
  if(svgUrl){
    const size = (typeof CELL !== 'undefined' ? CELL : 56) * 0.92;
    const img = document.createElementNS(ns,'image');
    img.setAttribute('class','piece-svg-img');
    img.setAttribute('x', String(-size/2));
    img.setAttribute('y', String(-size/2));
    img.setAttribute('width', String(size));
    img.setAttribute('height', String(size));
    img.setAttribute('href', svgUrl);
    img.setAttributeNS('http://www.w3.org/1999/xlink', 'href', svgUrl);
    img.setAttribute('preserveAspectRatio', 'xMidYMid meet');
    img.setAttribute('style','pointer-events:none');
    img.addEventListener('error', function(){ try{ img.style.display='none'; }catch(e){} });
    img.addEventListener('load', function(){
      try{ base.style.opacity='0'; rim.style.opacity='0'; glyph.style.opacity='0'; }catch(e){}
    });
...
async function confirmAndLockAiBet(){
  ...
  if(stake <= 0){
    ...
    await resetGame({ refundStake:false, skipAiStake:true });
    return true;
  }
  if(!getCoinIdentity()){
  ...
  try{
    ...
    await saveCoinStateToPlayer();
    refreshThemeLocks();
    setAiBetStatus('Đã khóa '+stake+' coin · số dư còn '+coinState.coins+' · lãi thắng ~'+profit+' + thưởng '+base+'.', false);
    try{ refreshThemeLocks(); }catch(e){}
    try{ if(typeof renderPlayerSessionUI==='function') renderPlayerSessionUI(); }catch(e){}
    try{ closeDrawer(); }catch(e){}
    // Không hoàn stake — bắt đầu ván với coin đã khóa
    await resetGame({ refundStake:false, skipAiStake:true });
    return true;
  }catch(e){
    state.aiStakeLocked = 0;
...
async function settlePveCoins(resultWinner){
  ...
  if(state.pveSettled) return;
  state.pveSettled = true;
  if(!getCoinIdentity()){
...
function ensureCheatDay(){
  ...
  if(coinState.cheatDate === d) return d;

  // Gộp cheatUses cũ (giftcode sai field) vào bonus nếu còn
  try{
    const legacy = Math.max(0, +(coinState.cheatUses||0));
    if(legacy > 0){
      coinState.cheatBonus = Math.max(0, +(coinState.cheatBonus||0)) + legacy;
      coinState.cheatUses = 0;
    }
  }catch(e){}

  const bonus = Math.max(0, +(coinState.cheatBonus||0));
  const used = Math.max(0, +(coinState.cheatUsed||0));
  const remaining = Math.max(0, CHEAT_DAILY_LIMIT - used) + bonus;

  coinState.cheatDate = d;
  if(remaining > CHEAT_DAILY_LIMIT){
    // > 20: không reset pool — giữ toàn bộ lượt còn lại
    coinState.cheatUsed = 0;
    coinState.cheatBonus = remaining;
  } else {
    // ≤ 20: reset ngày mới → lại 20 lượt free
    coinState.cheatUsed = 0;
    coinState.cheatBonus = 0;
  }
...
function refreshCheatUsesUI(){
  ...
  const usesRow = document.getElementById('cheatUsesRow') || (badge ? badge.closest('.cheat-uses-stack, .join-row') : null);
  if(isCheatUnlimited()){
    if(badge){ badge.style.display = 'none'; badge.textContent = ''; }
    if(buy) buy.style.display = 'none';
    if(usesRow) usesRow.style.display = 'none';
    if(st){
      st.textContent = 'Admin · không giới hạn lượt gian lận';
      st.className = 'online-status live';
    }
    return;
  }
  ...
  if(st){
    st.textContent = 'Còn '+left+' lượt (free '+dailyLeft+'/'+CHEAT_DAILY_LIMIT+(bonus?(' + mua/gift '+bonus):'')+'). Mua +'+CHEAT_BUY_USES+' = '+CHEAT_BUY_COST+' coin. >'+CHEAT_DAILY_LIMIT+' lượt: không reset ngày.';
    st.className = 'online-status' + (left <= 0 ? ' warn' : ' live');
  }
}

let _cheatConsumeLock = false;
async function consumeCheatUse(actionLabel){
  ...
  if(_cheatConsumeLock) return false;
  if(!getCoinIdentity()){
    setCheckInStatus('Đăng nhập kỳ thủ để dùng gian lận (20 lượt/ngày).', true);
    try{ refreshCheatUsesUI(); }catch(e){}
    return false;
  }
  _cheatConsumeLock = true;
  try{
    // Không reload giữa chừng nếu vừa trừ xong — tránh đọc số cũ / đếm kép
    ensureCheatDay();
    const left = cheatRemainingUses();
    if(left <= 0){
  ...
  if(!getCoinIdentity()){
    ...
    try{
      showCoinPopup({
        ...
        html:'<div class="coin-popup-hint">Bạn phải <b>đăng nhập kỳ thủ</b> để mua lượt gian lận.</div>',
        okLabel:'Đóng'
      });
    }catch(e){}
    return;
...
async function cheatSkipAiTurn(){
  ...
  renderMarkers();
  updateStatus();
  // Trừ lượt sau khi đã skip thành công
  try{ await consumeCheatUse('skip'); }catch(e){}
}

async function cheatKillPiece(r,c, opts){
  if(state.mode==='pvp' || state.online.active || state.gameOver) return false;
  opts = opts || {};
  const target = state.board[r] && state.board[r][c];
  if(!target) return false;
  // Chỉ ăn quân địch
  try{
    const aiColor = (typeof aiSideColor==='function') ? aiSideColor() : (state.humanColor==='red'?'black':'red');
    if(target.color !== aiColor) return false;
  }catch(e){}

  if(!opts.skipConsume){
    if(!(await consumeCheatUse('kill'))) return false;
  }
  ...
  try{ addHistoryEntry(state.history[state.history.length-1]); }catch(e){}
  try{ addCapturedChip(target); }catch(e){}
  renderPieces();
  renderMarkers();
  try{ sfxCapture(); }catch(e){}
...
async function cheatBeheadGeneral(){
  if(state.mode==='pvp' || state.online.active || state.gameOver) return;
  const enemy = (typeof aiSideColor==='function') ? aiSideColor() : (state.humanColor==='red'?'black':'red');
  const g = findGeneral(state.board, enemy);
  if(!g){
    try{ setCheckInStatus('Không tìm thấy tướng địch.', true); }catch(e){}
    return;
  }
  // Trừ 1 lượt rồi hạ tướng (kill không trừ thêm)
  if(!(await consumeCheatUse('behead'))) return;
  await cheatKillPiece(g.r, g.c, { skipConsume: true });
}
...
function finishWithCheatWin(){
  ...
  try{ sfxGameResult(state.humanColor, false); }catch(e){}
  try{ fireConfetti(); }catch(e){}
  try{ deleteFinishedSave(); }catch(e){}
...
async function cheatReviveChariot(){
  if(state.mode==='pvp' || state.online.active || state.gameOver) return;
  const color = state.humanColor || 'red';
  // Tìm ô trống gần phía mình
  let placed = false;
  for(let r=0;r<ROWS && !placed;r++){
    for(let c=0;c<COLS && !placed;c++){
      if(!state.board[r][c]){
        if(!(await consumeCheatUse('revive'))) return;
        const piece = {type:'chariot', color: color};
  ...
  if(!placed){
    try{ setCheckInStatus('Không còn ô trống để hồi xe.', true); }catch(e){}
  }
}
...
async function loadConfigAndInit(){
  ...
  // Một lần duy nhất: load coin → theme + piece set (tránh load bàn 2-3 lần)
  try{
    if(typeof loadCoinStateFromPlayer==='function'){
      loadCoinStateFromPlayer().then(function(){
        try{
          let theme = (typeof coinState!=='undefined' && coinState.preferredTheme) || null;
          if(!theme){ try{ theme = localStorage.getItem(THEME_STORAGE_KEY); }catch(e){} }
          if(theme && typeof applyTheme==='function'){
            applyTheme(theme, { force:true, preview:true });
          } else if(typeof loadSavedTheme==='function'){
            loadSavedTheme();
          }
        }catch(e){}
        try{
          const ps = (typeof coinState!=='undefined' && coinState.activePieceSet) || localStorage.getItem('cotuong_piece_set');
          if(ps && typeof applyPieceSet==='function' && typeof isPieceSetUnlocked==='function' && isPieceSetUnlocked(ps)){
            applyPieceSet(ps, { force:true, preview:true });
          }
        }catch(e){}
      }).catch(function(){
...
try{
  ...
  const mo = new MutationObserver(()=>{
    if(_stepperMoT) return;
    _stepperMoT = setTimeout(()=>{
      _stepperMoT = 0;
      try{ enhanceNumberSteppers(); }catch(e){}
    }, 120);
  });
