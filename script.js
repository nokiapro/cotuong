  ...
  if(typeof THEME_META !== 'undefined'){
    Object.keys(THEME_META).forEach(tid=>{
      ...
      out[id] = {
        ...
        name:'Giao diện '+meta.name, emoji:em, price:meta.price,
        desc:'Mở khóa giao diện bàn «'+meta.name+'»'
      };
    });
  }
  return out;
...
function renderShopList(){
  ...
  slice.forEach(it=>{
    ...
    const emoji = (it.emoji && String(it.emoji).trim()) ? it.emoji : '🎁';
...
async function adminLoadData(){
  ...
  try{
    ...
    if(roomsList) roomsList.innerHTML = '<div class="admin-empty">Lỗi tải phòng.).</div>';
