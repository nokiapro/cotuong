...
async function openMyClanModal(){
  try{
    ...
    members.forEach(mid=>{
      ...
      if(mid !== playerSession.id){
        actions = '<div class="clan-member-actions">'+
          ...
          (isOwner
            ? ('<select class="clan-role-select" data-mid="'+mid+'" title="Đặt chức" style="min-width:140px;max-width:180px;">'+
               '<option value="">— Đặt chức —</option>'+
               '<option value="owner2"'+(role==='owner2'?' selected':'')+'>Chủ Clan Cấp 2</option>'+
               '<option value="manager"'+(role==='manager'?' selected':'')+'>Quản Lý</option>'+
               '<option value="member"'+(role==='member'?' selected':'')+'>Thành Viên</option>'+
               '<option value="owner1">Chủ Clan Cấp 1 (chuyển quyền)</option>'+
               '</select>')
            : '')+
          '</div>';
      }
    ...
    body.querySelectorAll('.clan-role-select').forEach(sel=>{
